...
const createUsers = async () => {
  ...
  const promises = users.map(async (user, index) => {
    ...
    try {
      ...
      // User already exists in Auth table
      console.log(`User already exists in Auth table for ${email}`)
  ...
  console.log(`Creating admin if it doesn't exist`)
...
const getOrCreateAdminUser = async () => {
  try {
    console.log(`Creating admin if it doesn't exist`)
    const admin = await fetchUserByEmail('florian@instamobile.io')
    if (!admin) {
      const userRecord = await auth.getUserByEmail('florian@instamobile.io')
      const { uid } = userRecord
      const adminUser = {
        profilePictureURL:
    ...
    console.log(`Error thrown in getOrCreateAdminUserID`)
    return null
  }
}
